...
import { LocalSigner, ThalaAddLiquidityTool, ThalaRemoveLiquidityTool, ThalaStakeTokenTool, ThalaUnstakeTokenTool } from "move-agent-kit";
...
/**
 * Fetches yield opportunities from Thala Swap
 * @returns Array of yield opportunities with APY, risk, and other details
 */
export const fetchThalaSwapYields = async (tokenName:string) => {
    try {
      ...
      const stream = await agent.stream(
        {
          messages: [
            new HumanMessage(`List all current yield farming opportunities on Joule Finance with their APYs and details for token ${tokenName}.`)
          ],
        },
        config
      );
